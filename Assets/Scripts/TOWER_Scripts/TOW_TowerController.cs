...
namespace TOWER
{
    ...
    {
        [Header("Attack setup")] public TOW_ProjectileController projectilePrefab;
        ...
        private void Start()
        {
            _playerTransform = TOW_GameManager.Instance.playerTransform;
            stats.Initialize();
        }
        ...
        private void SpawnProjectile()
        {
            ...
            List<TOW_EnemyController> enemiesInRange =
                TOW_GameManager.Instance.GetEnemiesInRange(parentTransform.position, stats.range);
            if (enemiesInRange.Count > 0)
            {
                TOW_ProjectileController projectile = Instantiate(projectilePrefab, parentTransform.position,
